...
string ofApp::getPostUrl(const unsigned int sensor, const float data){
    string url = SERVER+"/sensors/postSensor?d=";
...
void ofApp::updateSensors(){
    ...
    if ((int)ofGetFrameNum()%frequency==0) {
        ...
        ofHttpResponse resp = ofLoadURL(getPostUrl(sensor, data));
        generalText->setTextString(resp.data);
    }
    
    //receive data
    if ((int)ofGetFrameNum()%frequency==floor(frequency/2)) {
        const string url = SERVER+"/sensors/getSensors";
